...
- Explorer service accounts, database secrets, Cloud SQL infrastructure,
  ...
  database role hardening, bearer-token `/api/records` agent API access, and
  app-level IAP JWT validation on protected routes are live.
...
The main remaining work is focused: keep manual DNS drift visible and repeat
focused live smoke checks after routing or auth changes.
...
- Deployed the direct Explorer record API on 2026-09-03. The load balancer now
  routes `/api/records` and `/api/records/*` to `explorer-api-web-backend`,
  CHM no longer proxies Explorer writes, and bearer-token smoke checks verified
  unauthenticated `401`, invalid-token `403`, writer-token read `200`,
  `validateOnly` write preflight `200`, and throwaway create/delete `200`.
...
| CHM-SEC-003 | High | Completed; deployed | CHM depends on `IAP_JWT_AUDIENCE` for app-level IAP JWT validation. Terraform sets it, and the app now fails creation in production if the value is missing. Deployed on 2026-09-03 as image `us-east4-docker.pkg.dev/chm-network/chm-apps/chm@sha256:a02418369050dfb52f5eba561df32628f6116ddc9c6f1a002d31ba7582c2c90e`; Cloud Run revision `chm-00013-lvp` is ready and serving 100% traffic. | Keep `NODE_ENV=production` and `IAP_JWT_AUDIENCE` in Terraform. Keep the fail-closed test in the CHM test suite. |
| CHM-SEC-004 | High | Mitigated; verified | Direct Cloud Run bypass appears controlled. Terraform restricts `chm`, `explorer`, `explorer-admin`, and `explorer-api` ingress so default `run.app` URLs cannot bypass the load balancer, IAP, or Explorer API bearer-token boundary. Live external checks on 2026-08-31 returned Google platform `404` responses for direct Explorer, Explorer admin, and Explorer API `run.app` URLs. | Keep Cloud Run ingress restricted. Periodically verify the default `run.app` URLs cannot serve CHM or Explorer admin without the load balancer/IAP path, cannot serve public Explorer outside the load balancer path, and cannot serve Explorer API outside `/api/records`. |
...
| CHM-SEC-010 | High | Completed; verified | Explorer database table-level grants are directionally correct, and runtime services use separate DB users: public `explorer` uses `explorer_read`; `explorer-api` uses `explorer_write`; explicit schema/setup work uses `explorer_schema_admin`. Browser writes now go through IAP-protected Explorer admin, and agent writes go through bearer-token `/api/records`; CHM does not forward write identity. In PostgreSQL, `public` is the default schema name inside the private database, not public internet access. Root cause: Cloud SQL-created users were members of `cloudsqlsuperuser`, and that inherited role had schema `CREATE`. On 2026-08-28, the live database fix removed `cloudsqlsuperuser`, `CREATEDB`, and `CREATEROLE` from `explorer_read`, `explorer_write`, and `explorer_schema_admin`; revoked database/schema `CREATE` from read/write and `PUBLIC`; preserved table read/write grants; moved `public` schema and all Explorer table ownership to `explorer_schema_admin`; and removed the old schema-setup identity. Cloud SQL row-count checks still verified `102` sources, `117` nodes, `139` edges, `10` ryu routes, and `2` saved views. | Keep the Ryu Postgres setup SQL as the source of truth for these grants. Re-run read/write/schema privilege probes after future Cloud SQL user recreation or role changes. |
| CHM-SEC-011 | Medium | Completed; verified | CHM no longer owns an Explorer write proxy in application code or load-balancer routing. Browser writes go directly to IAP-protected Explorer admin, and agent writes go directly to Explorer's bearer-token record API at `/api/records`. Terraform removed stale `/api/explorer` routing and CHM-to-API service invoker wiring on 2026-09-03. | Keep CHM out of Explorer write forwarding. Future app writes should either be owned by the target app or added as a deliberately scoped API route with its own auth boundary. |
...
| CHM-SEC-020 | Medium | Completed; verified | Explorer is split into public read-only `/explorer`, IAP-protected `/explorer/admin`, and bearer-token `/api/records`. Terraform explicitly disables IAP on `explorer-web-backend` and `explorer-api-web-backend`, enables IAP on `explorer-admin-web-backend`, keeps load-balancer-only Cloud Run ingress, runs public Explorer with `explorer_read`, and runs Explorer API with `explorer_write`. On 2026-09-03, `/explorer/` returned `200`, unauthenticated `/api/records` returned JSON `401 missing_bearer_token`, invalid bearer returned `403 invalid_bearer_token`, writer-token read returned `200`, `validateOnly` write preflight returned `200`, a throwaway record create/delete smoke returned `200` then `404` after cleanup, and `localeAvailability=available`, `missing`, `partial`, and `complete` each returned `200`. Explorer admin revision `explorer-admin-00008-qz9` serves 100% traffic with `IAP_JWT_AUDIENCE=/projects/288836337031/global/backendServices/5570063593656309274`; Explorer API revision `explorer-api-00017-2gh` serves 100% traffic. | Keep `/explorer` read-only and redacted. Keep `/explorer/admin` behind IAP with app-level JWT validation. Keep `/api/records` scoped to bearer-token auth with generous token rate limiting. If `explorer-admin-web-backend` is recreated, update `explorer_admin_iap_backend_service_id` and re-apply Terraform. |
...
- Latest CHM deploy on 2026-09-03 from commit `07dd201`: Cloud Run revision
  `chm-00013-lvp`, image
  `us-east4-docker.pkg.dev/chm-network/chm-apps/chm@sha256:a02418369050dfb52f5eba561df32628f6116ddc9c6f1a002d31ba7582c2c90e`
- Latest Explorer deploy on 2026-09-03: public/admin from Ryu commit
  `d6b6992`, API from Ryu commit `7bb5257`; public revision
  `explorer-00018-7qw`, admin revision `explorer-admin-00008-qz9`, API
  revision `explorer-api-00017-2gh`, public image
