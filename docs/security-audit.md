...
| CHM-SEC-011 | Medium | Refactored locally; deploy pending | CHM no longer owns an Explorer write proxy in application code. Browser writes should go directly to IAP-protected Explorer, and agent writes should go directly to Explorer's bearer-token record API. The previous CHM-forwarded user-context model is retained here only as historical audit context for the 2026-08-31 deployed revision. | Deploy the CHM proxy-removal image with the matching Explorer API refactor, then remove stale `/api/explorer` load-balancer and service-to-service wiring in a deliberate Terraform cleanup. |
