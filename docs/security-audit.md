...
- Enabled Explorer app-level IAP JWT validation by setting
  ...
  on Cloud Run, currently revision `explorer-00010-vw6`.
...
| CHM-SEC-020 | Medium | Completed; verified | Explorer's browser-facing service is protected by load-balancer IAP and restricted Cloud Run ingress. Terraform sets `IAP_JWT_AUDIENCE` to `/projects/288836337031/global/backendServices/4582439918390522076`, so Explorer performs app-level validation of the signed IAP assertion when requests reach the app. Live verification on 2026-08-31 showed backend service `explorer-web-backend` has ID `4582439918390522076` with IAP enabled, Cloud Run revision `explorer-00010-vw6` serves 100% traffic with the audience env var set, and unauthenticated `/explorer` requests are redirected by IAP before reaching Explorer. The deployed Explorer image is `us-east4-docker.pkg.dev/chm-network/chm-apps/explorer@sha256:070e49ba801cfa05f285ca18130c7b3ec7b835e5c28946457b334dcca94b7777`. | Keep the Explorer backend service ID in Terraform. If `explorer-web-backend` is recreated, update `explorer_iap_backend_service_id` and re-apply Terraform. |
...
- Latest Explorer deploy on 2026-08-31 from Ryu commit `fa36202`: Cloud Run
  revisions `explorer-00010-vw6` and `explorer-api-00009-27n`, image
