...
- Enabled Explorer app-level IAP JWT validation by setting
  ...
  on Cloud Run, currently revision `explorer-00011-kxh`.
...
| CHM-SEC-003 | High | Completed; deployed | CHM depends on `IAP_JWT_AUDIENCE` for app-level IAP JWT validation. Terraform sets it, and the app now fails creation in production if the value is missing. Deployed on 2026-08-31 as image `us-east4-docker.pkg.dev/chm-network/chm-apps/chm@sha256:a696db7b949cde2fe2c4fa9e65179b16383ea37f3942fa54b87981f1828e6ff3`; Cloud Run revision `chm-00009-wkz` is ready and serving 100% traffic. | Keep `NODE_ENV=production` and `IAP_JWT_AUDIENCE` in Terraform. Keep the fail-closed test in the CHM test suite. |
...
| CHM-SEC-020 | Medium | Completed; verified | Explorer's browser-facing service is protected by load-balancer IAP and restricted Cloud Run ingress. Terraform sets `IAP_JWT_AUDIENCE` to `/projects/288836337031/global/backendServices/4582439918390522076`, so Explorer performs app-level validation of the signed IAP assertion when requests reach the app. Live verification on 2026-08-31 showed backend service `explorer-web-backend` has ID `4582439918390522076` with IAP enabled, Cloud Run revision `explorer-00011-kxh` serves 100% traffic with the audience env var set, and unauthenticated `/explorer` requests are redirected by IAP before reaching Explorer. The deployed Explorer image is `us-east4-docker.pkg.dev/chm-network/chm-apps/explorer@sha256:cb1395e8fbf3707089f57367a5e5b3809a5f3a222a6387c729752aaf69e1edba`. | Keep the Explorer backend service ID in Terraform. If `explorer-web-backend` is recreated, update `explorer_iap_backend_service_id` and re-apply Terraform. |
...
- Latest CHM deploy on 2026-08-31 from commit `136db89`: Cloud Run revision
  `chm-00009-wkz`, image
  `us-east4-docker.pkg.dev/chm-network/chm-apps/chm@sha256:a696db7b949cde2fe2c4fa9e65179b16383ea37f3942fa54b87981f1828e6ff3`
- Latest Explorer deploy on 2026-08-31 from Ryu commit `a44fef4`: Cloud Run
  revisions `explorer-00011-kxh` and `explorer-api-00010-spd`, image
