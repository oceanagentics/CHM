...
- Enabled Explorer app-level IAP JWT validation by setting
  ...
  on Cloud Run, currently revision `explorer-00009-75l`.
...
| CHM-SEC-003 | High | Completed; deployed | CHM depends on `IAP_JWT_AUDIENCE` for app-level IAP JWT validation. Terraform sets it, and the app now fails creation in production if the value is missing. Deployed on 2026-08-31 as image `us-east4-docker.pkg.dev/chm-network/chm-apps/chm@sha256:29ea43ef4d384d5d1252f241202e6938829654d77e1682d1c756fbb296abdff5`; Cloud Run revision `chm-00008-vx8` is ready and serving 100% traffic. | Keep `NODE_ENV=production` and `IAP_JWT_AUDIENCE` in Terraform. Keep the fail-closed test in the CHM test suite. |
| CHM-SEC-004 | High | Mitigated; verified | Direct Cloud Run bypass appears controlled. Terraform restricts `chm`, `explorer`, and `explorer-api` ingress so default `run.app` URLs cannot bypass IAP or the private API boundary. Live external checks on 2026-08-31 returned Google platform `404` responses for direct CHM, Explorer, and Explorer API `run.app` URLs. | Keep Cloud Run ingress restricted. Periodically verify the default `run.app` URLs cannot serve CHM or Explorer without the load balancer/IAP path. |
...
| CHM-SEC-020 | Medium | Completed; verified | Explorer's browser-facing service is protected by load-balancer IAP and restricted Cloud Run ingress. Terraform sets `IAP_JWT_AUDIENCE` to `/projects/288836337031/global/backendServices/4582439918390522076`, so Explorer performs app-level validation of the signed IAP assertion when requests reach the app. Live verification on 2026-08-31 showed backend service `explorer-web-backend` has ID `4582439918390522076` with IAP enabled, Cloud Run revision `explorer-00009-75l` serves 100% traffic with the audience env var set, and unauthenticated `/explorer` requests are redirected by IAP before reaching Explorer. The deployed Explorer image is `us-east4-docker.pkg.dev/chm-network/chm-apps/explorer@sha256:e2b744ed43b60f99e6740e5e2a156c8bffc39192a75ff7a2af6dac17e471f29e`. | Keep the Explorer backend service ID in Terraform. If `explorer-web-backend` is recreated, update `explorer_iap_backend_service_id` and re-apply Terraform. |
...
1. Verify the signed-in CHM/Explorer review form in a browser session. Backend
   review writes and denial cases are already verified.
...
- Latest Explorer deploy on 2026-08-28 from Ryu commit `6a03086`; Cloud Build
  `0f8f6ce5-79af-407d-b4e7-6c15382cbb6b`
- Latest CHM deploy on 2026-08-31: Cloud Run revision `chm-00008-vx8`, image
  `us-east4-docker.pkg.dev/chm-network/chm-apps/chm@sha256:29ea43ef4d384d5d1252f241202e6938829654d77e1682d1c756fbb296abdff5`
- Latest Explorer deploy on 2026-08-31 from Ryu commit `6a03086`: Cloud Run
  revisions `explorer-00009-75l` and `explorer-api-00008-mhc`, image
...
- Review API probe on 2026-08-31: temporary Cloud Run job
  `explorer-api-review-probe-20260831` ran as `chm-sa`, updated `fishbase` as
